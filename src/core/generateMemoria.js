...
// ── Análisis longitudinal (bastones a lo largo) de una trabe con perfil por estaciones ──
function longitudinalHTML(t) {
  if (!t.perfil?.members?.length) return ''
  let G = null
  try { G = analyzeGroup(t, t.perfil) } catch { return '' }
  if (!G || !G.results.some((r) => r.L > 0)) return ''
  const bars = (arr, cal) => arr.length ? arr.map((b) => `${b.k}#${esc(cal)} L=${fmt(b.len, 2)} m a ${fmt(b.x0, 2)} m del apoyo I${b.ancla ? ` (anclar en ${b.ancla})` : ''}`).join('; ') : 'sin bastón'
  const estado = (r) => r.status === 'insuficiente' ? '<span style="color:#b91c1c;font-weight:700">INSUFICIENTE</span>'
